...
void ARobotSpectatorPawn::BeginPlay()
{
	...
	ensureMsgf(CachedPC, TEXT("RobotSpectatorPawn requires a valid PlayerController at BeginPlay."));
	if (!CachedPC)
	{
		return;
	}
...
void ARobotSpectatorPawn::HandleDropOrAttach(const FHitResult& Hit)
{
	if (!DraggedActor)
	{
		return;
	...
	if (Point && DraggedPart)
	{
		if (Point->TryAttachPart(DraggedPart))
		{
			...
			{
				// Should not happen if we started dragging correctly.
...
void ARobotSpectatorPawn::EndDragAndResumeHover(bool bClearDraggedHighlight)
{
	if (!DraggedActor)
	{
		return;
...
void ARobotSpectatorPawn::StartHighlightTimer()
{
	...
	GetWorldTimerManager().SetTimer(
		...
		this,
		&ARobotSpectatorPawn::UpdateCurrentTarget,
		0.1f,
		true
	);
}
...
void ARobotSpectatorPawn::SetCurrentTarget(AActor* NewTarget)
{
...
void ARobotSpectatorPawn::UpdateCurrentTarget()
{
	if (DraggedActor)
	{
		return;
...
void ARobotSpectatorPawn::SetHighlightIfPresent(AActor* Actor, bool bOn)
{
	if (!Actor)
	{
		return;
...
void ARobotSpectatorPawn::CreateArmStatusWidget()
{
	check(CachedPC);
